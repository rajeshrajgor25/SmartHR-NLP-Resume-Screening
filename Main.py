...
if __name__ == '__main__':
    ...
    for mail in emails:
        if not is_candidate_email(mail):
            print(f"⚠️ Skipping non-candidate email from {mail.get('from')}")
            continue
        ...
        skill_match_ratio = (match_count / total_required) if total_required else 0

        shortlisted_flag = (
            (total_required > 0 and match_count >= total_required - 1) or
            (best_score >= 0.60)
        )
        ...
        if shortlisted_flag:
            ...
            send_shortlist_email(candidate['email'], 'Candidate', 'the applied position')
        else:
            rejected.append(candidate)
            send_rejection_email(candidate['email'], 'Candidate', 'the applied position')

        print(f"{candidate['email']} | JD Match: {candidate['similarity']}% | Skills: {match_count}/{total_required} | Shortlisted: {candidate['shortlisted']}")

    if shortlisted:
        df_short = pd.DataFrame(shortlisted)
        save_to_excel(df_short, OUTPUT_EXCEL)
        print(f'✅ Shortlisted candidates saved to {OUTPUT_EXCEL}')
    else:
        print('⚠️ No candidates shortlisted.')

    if rejected:
        df_reject = pd.DataFrame(rejected)
        save_to_excel(df_reject, "rejected_candidates.xlsx")
...
if __name__ == '__main__':
    ...
    for mail in emails:
        ...
        print(f"{candidate['email']} | JD Match: {candidate['similarity']}% | Skills: {match_count}/{total_required} | Exp: {exp_years} yrs | Shortlisted: {candidate['shortlisted']}")
